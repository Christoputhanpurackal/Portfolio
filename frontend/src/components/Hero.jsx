...
export default function Hero() {
  ...
  if (!profile) return null; // Wait for data to load
        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4">
              ...
              <a href={`${API_BASE_URL}/download-cv?t=${cacheBuster}`} target="_blank" rel="noopener noreferrer" className="px-8 py-3 rounded-full glass hover:bg-white/10 font-medium flex items-center transition-colors">
          ...
          >
            <div className="relative w-72 h-72 md:w-96 md:h-96 rounded-full p-2 glass">
              <div className="w-full h-full rounded-full overflow-hidden border-4 border-white/10 relative">
                <img 
                  src={imgError ? "https://ui-avatars.com/api/?name=" + encodeURIComponent(profile.name) + "&size=512&background=0D8ABC&color=fff" : `${API_BASE_URL}/static/profile.jpg?t=${cacheBuster}`} 
                  alt={profile.name} 
